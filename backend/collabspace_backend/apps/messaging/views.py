...
class ChannelViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # 1. For detail views/actions, return specific channel if member
        if self.action in ['retrieve', 'update', 'partial_update', 'destroy', 'add_member', 'remove_member', 'archive', 'unarchive', 'members']:
            return Channel.objects.filter(
                members=user,
                is_deleted=False,  
            ).select_related('created_by').prefetch_related('members')

        # 2. FIX: For LIST views, return all channels the user is part of
        # This prevents the "NoneType has no len()" error
        ...
        # Optional: Filter by workspace if provided in query params
        workspace_id = self.request.query_params.get('workspace') or self.request.query_params.get('workspace_id')
        if workspace_id:
    ...
    def add_member(self, request, pk=None):
        ...
        try:
            ...
            # Check if user is part of the workspace (assuming related_name is 'workspacemember_set' or similar)
            # Adjust 'workspacemember_set' to match your exact User-Workspace relation
            if not user_to_add.workspace_memberships.filter(workspace=channel.workspace, is_active=True).exists():
...
class MessageViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        # For detail views and custom actions, return all non-deleted messages
        # Permission checking happens in has_object_permission
        if self.action in ['retrieve', 'update', 'partial_update', 'destroy', 'add_reaction', 'remove_reaction', 'pin', 'unpin']:
            return Message.objects.filter(
                is_deleted=False
            ).select_related('sender', 'parent_message__sender').prefetch_related('reactions', 'mentions', 'replies')
        
        # For list view, require channel query parameter
    ...
    def search(self, request):
        ...
        messages = Message.objects.filter(
            ...
            is_deleted=False
        ).select_related('sender').prefetch_related('reactions', 'mentions')[:50]
...
class DirectMessageViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            raise ValidationError({"recipient": "Recipient user not found."})
            
        if self.request.user == recipient:
            raise ValidationError({"recipient": "Cannot send DM to self."})
    ...
    def conversations(self, request):
        ...
        # Optimized query using simple GROUP BY approach logic via ORM
...
class MessageSearchView(APIView):
    ...
    def get(self, request):
        ...
        messages = Message.objects.filter(
            ...
            content__icontains=query,
            is_deleted=False
        ).select_related('channel', 'sender').order_by('-created_at')[:50]
