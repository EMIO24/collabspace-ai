...
class AIViewMixin:
    ...
    def handle_ai_call(self, service_method, serializer_class, *args, **kwargs):
        ...
        try:
            # 2. Service Call
            result = service_method(user, *args, **validated_data, **kwargs)
            return Response(result, status=status.HTTP_200_OK)

        except Exception as e:
            # 3. General error handling (AI API + internal errors)
            return Response(
...
class TaskAIView(AIViewMixin, viewsets.GenericViewSet):
    ...
    def breakdown(self, request):
        ...
        def service_wrapper(user):
            ...
            if auto_create:
                pass  # Placeholder: save subtasks
    ...
    def action_items(self, request):
        def service_wrapper(user, transcript, auto_create_tasks=False, project_id=None):
            ...
            if auto_create_tasks:
                pass  # Placeholder: save action items as tasks
...
class AssistantView(AIViewMixin, viewsets.GenericViewSet):
    ...
    def chat(self, request):
        ...
        try:
            ...
            return Response({'error': 'CHAT_API_ERROR', 'message': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
